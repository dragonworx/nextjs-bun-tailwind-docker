    ...
    fetch("/api/routes")
      ...
      .then((data) => {
        if (data && data.routes) {
          ...
          const navigationRoutes = data.routes.filter(
            (route: Route) =>
          ...
          <div className="text-white text-xl sm:text-2xl font-black flex items-center gap-3 group-hover:scale-105 transition-transform duration-300">
            ...
            <span className="bg-gradient-to-r from-cyan-400 via-blue-400 to-purple-400 bg-clip-text text-transparent animate-pulse">
              ...
              <img
                src="/bike.jpg"
                alt="Bike"
                className="inline-block w-6 h-6 sm:w-8 sm:h-8 rounded-full object-cover ml-2"
              />
            </span>
          ...
          {routes.map((route, index) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`text-lg transition-transform duration-300 group-hover:scale-110 ${isActive ? "animate-bounce" : ""}`}
                >
